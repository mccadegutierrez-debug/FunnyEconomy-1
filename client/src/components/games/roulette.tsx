...
export default function Roulette() {
  ...
  const [betType, setBetType] = useState<'red' | 'black' | 'green' | 'odd' | 'even' | 'high' | 'low' | null>(null);
  ...
  const playMutation = useMutation({
    mutationFn: async ({ betAmount, selectedBetType }: { betAmount: number; selectedBetType: string }) => {
      const res = await apiRequest("POST", "/api/games/roulette", { bet: betAmount, betType: selectedBetType });
      return res.json();
    },
    onSuccess: (data) => {
      setIsSpinning(true);
    ...
    <div className="space-y-6">
      <Card className="glow-secondary border-secondary/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-impact text-3xl text-secondary">ROULETTE</CardTitle>
          <CardDescription>Spin the wheel and bet on colors or numbers!</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Card className="bg-muted p-6">
            <div className="flex justify-center mb-6">
              <div
                ...
                style={{
                  background: gameResult && !isSpinning 
                    ? gameResult.color === 'red' ? 'linear-gradient(45deg, #dc2626, #ef4444)' 
                    : gameResult.color === 'black' ? 'linear-gradient(45deg, #1f2937, #374151)'
          ...
          <div className="space-y-4">
            <Label className="text-center block text-lg font-semibold">Place Your Bet</Label>
          ...
          <div className="space-y-4">
            ...
            <Button
              onClick={handleSpin}
              disabled={!betType || isSpinning || playMutation.isPending || !user || user.coins < bet}
              ...
              <CardContent className="p-6 text-center">
                ...
                <div className="space-y-2 mb-4">
                  <div className="text-lg">
                    Ball landed on: <span className="font-bold">{gameResult.number}</span>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Color: {gameResult.color.toUpperCase()} | Your bet: {gameResult.betType.toUpperCase()}
                  </div>
                </div>
