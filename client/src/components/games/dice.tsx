...
export default function Dice() {
  ...
  const [prediction, setPrediction] = useState<'over' | 'under' | 'seven' | null>(null);
  ...
  const playMutation = useMutation({
    mutationFn: async ({ betAmount, selectedPrediction }: { betAmount: number; selectedPrediction: 'over' | 'under' | 'seven' }) => {
      const res = await apiRequest("POST", "/api/games/dice", { bet: betAmount, prediction: selectedPrediction });
      return res.json();
    },
    onSuccess: (data) => {
      setIsRolling(true);
    ...
    <div className="space-y-6">
      <Card className="glow-primary border-primary/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-impact text-3xl text-primary">DICE GAME</CardTitle>
          <CardDescription>Roll two dice - predict over 7, under 7, or exactly 7!</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <Label className="text-center block text-lg font-semibold">Make Your Prediction</Label>
          ...
          <div className="space-y-4">
            ...
            {prediction && (
              <div className="text-center text-sm text-muted-foreground mb-4">
                <p>
                  ...
                  (Win {prediction === 'seven' ? bet * 5 : Math.floor(bet * 1.9)} coins)
                </p>
              </div>
            )}
            ...
            <Button
              onClick={handleRoll}
              disabled={!prediction || isRolling || playMutation.isPending || !user || user.coins < bet}
              className="w-full font-comic text-lg bg-primary hover:bg-primary/80 glow-primary"
              data-testid="button-roll-dice"
            >
              {isRolling ? "ROLLING..." : playMutation.isPending ? "Loading..." : `ROLL DICE! (${bet} coins)`}
            </Button>
          </div>
              ...
              <CardContent className="p-6 text-center">
                ...
                <div className="space-y-2 mb-4">
                  <div className="text-lg">
                    Rolled: <span className="font-bold">{gameResult.dice1} + {gameResult.dice2} = {gameResult.total}</span>
                  </div>
